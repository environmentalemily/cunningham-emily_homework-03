...
**Leaf dry weight is strongly postively correlated with total pass (Pearson's R is 0.46).**  
...
**On average, total biomass differs between species. Gumweed, california poppy, and purple needlegrass appear to have the highest biomass. Deerweed, scarlet bulgar, and purple sage have the lowest biomass.**
...
**Total biomass appears to be greater when plants are well- watered.**
...
**Maybe slight positive correletation: as specific leaf area increases, so does total biomass. Correltation moderate (or strong depending on how conservative you want to be) is 0.453**
...
plot(model3)
#look at diagnostics for this model too!!
```
...
```{r}
model.sel(model0, #takes model objects and creates a table of the models
          model1, 
          model2, 
          model3)
```
...
**reference for water_treatment:** **Drought- stressed is the reference for water- treatment**  
...
- SLA slope: **for each 1 unit increase in SLA (m^2/g), you would expect a decrease in plant mass of 0.002 +/- (figure out how to display plus minus in doc) 0.0005, NO EFFECT OF SLA ON MASS EVEN THOUGH CHOSEN AS BEST MODEL**  
- well watered estimate: **ON AVERAGE ACROSS SPECIES AND SLA, well watered plants tend to be 0.12 +/- o.02 grams heavier than drought- stressed plants**    
